...
export default async function PropertyDetailsPage(props: { params: Promise<{ id: string, slug: string }> }) {
  ...
  // Handle SEO optimally: if the slug is incorrect or malformed in the URL, redirect to the correct one
  ...
  const locParts = [
    ...
    property.address?.area?.district?.name
  ].filter(Boolean);
